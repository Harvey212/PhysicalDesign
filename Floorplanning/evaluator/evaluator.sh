...
if [ $# -eq 4 ]; then
    ...
    echo "[Eval] usage1: bash evaluator.sh <input.block> <input.net> <output.rpt> <alpha>" # <runtime(s)>"
    #echo "[Eval] usage2: bash evaluator.sh <input.block> <input.net> <output.rpt> <alpha> #And input your runtime(s)."
    exit 1
fi
...
# check files
BASEDIR=$(dirname "$0")
FILE_LIST=("$BASEDIR/checker" "$BASEDIR/pd22_results.list" $1 $2 $3)
for FILE_NAME in ${FILE_LIST[@]}; do
    if [ ! -f $FILE_NAME ]; then
        echo "[Eval] error: Please check file '$FILE_NAME' exist."
        echo "(ps. checker and pd22_result.list should be in the same dir with evaluator.sh)"
        exit 1
    fi
done
# check alpha
